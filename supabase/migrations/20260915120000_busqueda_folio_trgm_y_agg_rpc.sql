-- Alineado al esquema real: ids bigint, notas_credito.serie_folio/cliente/usuario_vendedor_pv.
-- En SQL Editor de Supabase ejecuta PRIMERO:
...
-- 1) Prefijo/igualdad en folio (rápido de crear — prioriza esto si hubo timeout)
CREATE INDEX IF NOT EXISTS idx_notas_serie_folio_lower_pattern
  ON public.notas_credito (lower(serie_folio) text_pattern_ops);

-- 2) Contains ILIKE %…% (pueden tardar varios minutos en tablas grandes)
...
BEGIN
  ...
  IF v_q IS NOT NULL THEN
    ...
    v_folio_like := v_q !~ '\s' AND v_q ~ '^[A-Za-z0-9]';
  END IF;
  ...
  WITH filtered AS MATERIALIZED (
    ...
    WHERE (v_empresa IS NULL OR n.empresa = v_empresa)
      ...
      AND (
        ...
        OR (
          CASE
            WHEN v_folio_like THEN (
              lower(n.serie_folio) = lower(v_q)
              OR lower(n.serie_folio) LIKE lower(v_q) || '%'
            )
            ELSE (
