-- Antigüedad = fecha_nota (igual que el listado "Todas las notas").
-- Antes COALESCE(fecha_resolucion, ...) excluía notas viejas resueltas/importadas hace poco.
...
DECLARE
  ...
  v_corte date;
  v_out jsonb;
  v_total_cerradas integer := 0;
  v_con_resolucion integer := 0;
BEGIN
  ...
  muestra AS (
    SELECT jsonb_agg(row_to_json(s)) AS items
    FROM (
      SELECT n.id, n.serie_folio, n.fecha_nota, n.estado, n.fecha_resolucion
      FROM public.notas_credito n
      WHERE n.estado IN ('RESUELTA', 'CANCELADA')
        AND n.fecha_nota <= v_corte
        AND (v_empresa IS NULL OR n.empresa = v_empresa)
      ORDER BY n.fecha_nota ASC, n.id ASC
      LIMIT 5
    ) s
  )
  SELECT jsonb_build_object(
    ...
    'fecha_corte', v_corte,
    'empresa', v_empresa,
    'criterio_fecha', 'fecha_nota',
    'diag_total_cerradas', v_total_cerradas,
    'diag_con_fecha_resolucion', v_con_resolucion,
    'diag_muestra', COALESCE((SELECT items FROM muestra), '[]'::jsonb)
  )
  INTO v_out;
