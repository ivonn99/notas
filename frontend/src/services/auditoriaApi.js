  ...
  list: async (params = {}) => {
    if (isSupabaseConfigured) {
      ...
      let query = supabase
        ...
        .select('id, created_at, usuario_id, username, accion, entidad, entidad_id, detalle, ip, user_agent, request_id')
      ...
      if (q) {
        ...
        items = items.filter((it) => JSON.stringify(it.detalle || {}).toLowerCase().includes(needle) || String(it.entidad || '').toLowerCase().includes(needle) || String(it.entidad_id || '').toLowerCase().includes(needle))
      }
